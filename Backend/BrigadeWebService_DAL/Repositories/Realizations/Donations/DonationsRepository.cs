...
namespace BrigadeWebService_DAL.Repositories.Realizations.Donations
{
    public class DonationsRepository : RepositoryBase<Donation>, IDonationsRepository
    {
        ...
        public async Task<IEnumerable<Donation>> GetAllWithImageAsync(CancellationToken ct = default) =>
        await _db.Donations
            .AsNoTracking()
            .Include(d => d.Image)
            .ToListAsync(ct);
        public Task<Donation?> GetByIdWithImageAsync(int id, CancellationToken ct = default) =>
        _db.Donations
           .AsNoTracking()
           .Include(d => d.Image)
